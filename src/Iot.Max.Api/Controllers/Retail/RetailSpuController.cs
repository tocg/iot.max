...
namespace Iot.Max.Api.Controllers.Retail
{
    ...
    [Route("retail/spu")]
    [ApiController]
    public class RetailSpuController : ControllerBase
    {
        ...
        public IActionResult Query(int page = 1, int limit = 20, string search = "")
        {
            ...
            try
            {
                ...
                var par = new PageParameters<RetailSPU>();
                par.Proc = new PageProc { ProcName = "pr_retail_spu_list", ProcParm = parm, ProcOutName = "count" };
        ...
        public IActionResult Query(string id)
        {
            ...
            try
            {
                ...
                result.Data = _services.QueryFirst<RetailSPU>(id);
                result.Count = 1;
            }
            catch (Exception ex)
            {
        ...
        public IActionResult Delete(List<string> ids)
        {
            ...
            try
            {
                List<RetailSPU> parm = new List<RetailSPU>();
                foreach (var item in ids)
                {
                    parm.Add(new RetailSPU { ID = item });
                }
                var i = _services.Delete<RetailSPU>(parm);
