...
namespace Iot.Max.Api.Controllers.Rbac
{
    ...
    {
        private readonly ILog log = LogManager.GetLogger(Startup.repository.Name, typeof(RbacMenuController));
        private readonly DapperClientHelper _dapper;
        readonly IServices _services;
        public RbacMenuController(IDapperFactory dapperFactory,IServices services)
        {
        ...
        public IActionResult Query(int page = 1, int limit = 20, string search = "")
        {
            ...
            try
            {
                ...
                var par = new PageParameters<RbacMenu>();
                par.Proc = new PageProc { ProcName = "pr_rbac_menu_list", ProcParm = parm, ProcOutName = "count" };
        ...
        public IActionResult Query(string id)
        {
            ...
            try
            {
                ...
                result.Data = _services.QueryFirst<RbacMenu>(id);
                result.Count = 1;
            }
            catch (Exception ex)
            {
        ...
        public IActionResult Delete(List<string> ids)
        {
            ...
            try
            {
                List<RbacMenu> parm = new List<RbacMenu>();
                foreach (var item in ids)
                {
                    parm.Add(new RbacMenu { ID = item });
                }
